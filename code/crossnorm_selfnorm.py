...
def calc_ins_mean_std(x, eps=1e-5):
    ...
    std = var.sqrt().view(N, C, 1, 1, 1)
    mean = x.contiguous().view(N, C, -1).mean(dim=2).view(N, C, 1, 1, 1)
    return mean, std
...
class SelfNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        statistics = torch.cat((mean.squeeze(3).squeeze(3), std.squeeze(3).squeeze(3)), -1)

        g_y = self.g_fc(statistics)
        g_y = self.g_bn(g_y)
        g_y = torch.sigmoid(g_y)
        g_y = g_y.view(b, c, 1, 1, 1)

        if self.f_fc is not None:
            f_y = self.f_fc(statistics)
            f_y = self.f_bn(f_y)
            f_y = torch.sigmoid(f_y)
            f_y = f_y.view(b, c, 1, 1, 1)
...
if __name__ == '__main__':
    ...
    input = torch.rand(32, 3, 128, 128,32)
    output = block(input)
    print(input.size())
    print(output.size())