...
#from mamba import MambaLayerSN
from crossnorm_selfnorm import SelfNorm
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        # mamba = MambaLayer(init_features)
        # self.features.add_module(f"mambablock{0}", mamba)
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i == len(block_config) - 1:
                ...
                #self.temp.add_module(f"transition{i + 1}", trans)
                in_channels = _out_channels

            self.blocks.append(self.features)
            self.features = nn.Sequential()
                #@self.temp = nn.Sequential()

        # pooling and classification
        # self.pool64 = nn.Sequential(
        #     OrderedDict(
        #         [
        #             ("norm5", SelfNorm(chan_num=64)),
        #             #("norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=128)),
        ...
        self.pool128 = nn.Sequential(
            OrderedDict(
                [
                    ("norm5", SelfNorm(chan_num=128)),
                    #("norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=128)),
                    ("relu", get_act_layer(name=act)),
                    ("pool", avg_pool_type(1)),
                    #("flatten", nn.Flatten(1)),
        ...
        self.pool256 = nn.Sequential(
            OrderedDict(
                [
                    #("norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=256)),
                    ("norm5", SelfNorm(chan_num=256)),
                    ("relu", get_act_layer(name=act)),
                    ("pool", avg_pool_type(1)),
                    #("flatten", nn.Flatten(1)),
                ]
        ...
        self.pool512 = nn.Sequential(
            OrderedDict(
                [
                    ("norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=512)),#\("norm5", SelfNorm(chan_num=128)),
                    #("norm5", SelfNorm(chan_num=512)),
                    ("relu", get_act_layer(name=act)),
                    ("pool", avg_pool_type(1)),
                    #("flatten", nn.Flatten(1)),
                ]
        ...
        self.pool1024 = nn.Sequential(
            OrderedDict(
                [
                    ("norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=1024)),
                    #("norm5", SelfNorm(chan_num=1024)),
                    ("relu", get_act_layer(name=act)),
                    ("pool", avg_pool_type(1)),
                    #("flatten", nn.Flatten(1)),
                ]
        ...
        self.class_layers = nn.Sequential(
            OrderedDict(
                [
                    #("norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=1408)),
                    #("norm5", SelfNorm(chan_num=1408)),
                    ("flatten", nn.Flatten(1)),
                   ("Linear ",nn.Linear(128+256+512, out_channels)) 
                ]
            ))
        ...
        #self.vit  = ViT(hidden_size=896, mlp_dim=3076, num_layers=2, num_heads= 16)
